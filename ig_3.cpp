...
public:
    ...
    bool Init()//метод инициализации
    {
        ...
        m_pEffect->SetTextureUnit(0);//установка текстуры
    ...
    virtual void RenderSceneCB()//рендер сцены
    {
    ...
    virtual void SpecialKeyboardCB(int Key, int x, int y)//обработчик событий клавиатуры
    {
        m_pGameCamera->OnKeyboard(Key);
    }


    virtual void KeyboardCB(unsigned char Key, int x, int y)//обработчик событий клавиатуры
    {
        switch (Key) {
        case 'q':
            glutLeaveMainLoop();
